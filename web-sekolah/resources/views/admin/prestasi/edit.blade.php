        ...
        <div class="section-body">
            ...
            <div class="card">
                <div class="card-header">
                    <h4><i class="fas fa-trophy"></i> Edit Prestasi</h4>
                </div>
                    ...
                    <form action="{{ route('admin.prestasi.update', $prestasi->id) }}" method="POST"
                        ...
                        {{-- <div class="form-group">
                            ...
                            <select class="form-control select-category @error('category_id') is-invalid @enderror"
                                ...
                                @foreach ($categories as $category)
                                    @if($prestasi->category_id == $category->id)
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control content @error('content') is-invalid @enderror" name="content"
                                ...
                                rows="10">{!! old('content', $prestasi->content) !!}</textarea>
                            @error('content')
                            <div class="invalid-feedback" style="display: block">
                                {{ $message }}
                            </div>
                        ...
                        {{-- <div class="form-group">
                            ...
                            <select class="form-control" name="tags[]"
                                multiple="multiple">
                                @foreach ($tags as $tag)
                                    <option value="{{$tag->id}}" {{ in_array($tag->id, $prestasi->tags()->pluck('id')->toArray()) ? 'selected' : '' }}> {{ $tag->name }}</option>
                                @endforeach
                            </select>
                        </div> --}}
